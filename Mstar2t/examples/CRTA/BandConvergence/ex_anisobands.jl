...
titles = [L"$σ$ vs gap, $T$ = %$T, $μ$ = %$μ, $τ = const$",
          L"$S$ vs gap, $T$ = %$T, $μ$ = %$μ, $τ = const$",
          L"$n$ vs gap, $T$ = %$T, $μ$ = %$μ, $τ = const$"];
...
annotations = Dict(L"μ" => [(0.11,2.1e6),(0.11,50),(0.11,5e6)], 
                   L"E_V" => [(0.01,2.1e6-1e4),(0.01,49),(0.01,5e6-5e4)]);

fig = plot(3, gap, [σ,S*1e6,n], titles=titles, xlabels=xlabels, ylabels=ylabels, vlines=[0.0,0.1], color=mypalette[3], annotations=annotations);

savefig(joinpath(savepath, "ex_three_anisobands.png"), fig);
